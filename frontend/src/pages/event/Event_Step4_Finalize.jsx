...
const Event_Step4_Finalize = () => {
  ...
  const handleSubmit = async (e) => {
    e?.preventDefault?.();
    const newErrors = {};
    if (!agreements.organizer) newErrors.organizer = "Please accept the agreement";
    ...
    try {
      await saveOnboardingStep("event", "step4", { agreements });
      const stored = JSON.parse(localStorage.getItem("onboardingData"));
      const payload = buildOnboardingCompletePayload("event");
      const response = await apiPost("/api/onboarding/complete", payload);
      persistDashboardUser("event", stored, response?.data?.user);
          ...
          <div className="mb-14">
            ...
            <div className="space-y-8 px-2">
              ...
              <div className="flex gap-6 items-start group cursor-pointer" onClick={() => toggleAgreement('organizer')}>
                ...
                {errors.organizer && <p className="text-red-500 text-[10px] mt-1">{errors.organizer}</p>}
              </div>
